...
for(list<pair<int, int> >::iterator it = adj[id].begin(); it != adj[id].end(); it++){
        ...
        //{
            ...
            auxVerificaBipartido(vetor2,it->first,j,id,verifica);//A função se chama, fazendo um metodo recursivo,atualizando os paramtros da função
...
void grafo::deletarAresta(int id,int id2, int peso)
{
    if(verificaIdExiste(id)) //Verifica se o id passado existe no grafo
    {
	    ...
	    for(list<pair<int, int> >::iterator it = adj[id].begin(); it != adj[id].end(); it++){
            ...
            //{
                if(it->first == id2 && it->second == peso)//Verifica se o id e o peso da aresta que se deseja remover são iguais aos passados como parametro
                {
                    adj[vetor[id]].remove(*it);//A aresta desejada é removida
                    return;
                }
            }
        }
...
int* grafo::auxbuscaProfundidadeTransitivo(int* vetor3,int id1,int id2,int* j)
{
	...
	for(list<pair<int, int> >::iterator it = adj[vetor[id1]].begin(); it != adj[vetor[id1]].end(); it++){
        ...
        //{
            ...
            auxbuscaProfundidadeTransitivo(vetor3,vetor[it->first],vetor[id2],j);//A função se chama, fazendo um metodo recursivo,atualizando os paramtros da função
